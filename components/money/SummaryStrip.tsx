...
export function SummaryStrip({
    ...
    // Mobile only: the pinned chin opens to one line, so the list keeps the screen.
    const collapsible = expanded ? "" : "max-sm:hidden";
    ...
    return (
        <>
            ...
            >
                <div
                    id={headerId}
                    className={`flex flex-wrap items-center justify-between gap-x-3 gap-y-1 border-b bg-muted/40 px-4 py-2 text-[11px] text-muted-foreground ${collapsible}`}
                >
                    <p>
                ...
                <button
                    ...
                    aria-controls={`${headerId} ${panelId}`}
                    onClick={() => setExpanded((open) => !open)}
                    className="flex w-full items-center gap-2 px-4 py-3 text-left text-sm focus-visible:outline-2 focus-visible:outline-ring sm:hidden"
                >
                    <span className="min-w-0 flex-1 truncate font-semibold tabular-nums">
                        {`My cost ${formatMxn(summaryCost(totals).amount)} · Charged ${formatMxn(totals.charged.amount)}`}
                    </span>
                ...
                {/* Capped on mobile, so an opened list scrolls inside the panel
                    and the toggle above it never leaves the screen. */}
                <div
                    id={panelId}
                    className={`max-sm:max-h-[calc(100dvh-14rem)] max-sm:overflow-y-auto sm:flex sm:flex-wrap ${collapsible}`}
                >
                    ...
                    <FundingSplit
                        fromSavings={totals.notFromIncome.fromSavings.amount}
                        reimbursed={totals.notFromIncome.reimbursed.amount}
                        rows={fundingRows}
                        className="border-b px-4 py-2 sm:order-last sm:basis-full sm:border-t sm:border-b-0"
                    />
